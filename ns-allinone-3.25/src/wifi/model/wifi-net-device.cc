...
//added by Najwan
#include "ns3/frame-type-tag.h"
#include "ns3/qos-tag.h"
#include "ns3/node-id-tag.h"
#include "ns3/dev-id-tag.h"
#include <fstream> 				//To use with std::ofstream
#include "ns3/seq-ts-header.h"	//To be get SetTsHeader
#include "ns3/simulator.h"		//To use Simulator::Now().GetSeconds()
...
WifiNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  //Get the NodeId index and add NodeIdTag to packet
  //==============================================
  Ptr<Node> n = this->GetNode();		//check which node does this device belongs to
	
  QosTag qosTag;
  FrameTypeTag frametypeTag;
  
  //Check if this packet has a QoSTag and FrametypeTag
  if ((packet->PeekPacketTag(qosTag)) && (packet->PeekPacketTag(frametypeTag))){
				
		NodeIdTag nodeIdTag;
		nodeIdTag.SetNodeId(n->GetId());
		//std::cout << "WifiNetDevice Get Id: " << this->GetNode()->GetId() << std::endl;
		packet->AddPacketTag(nodeIdTag);
		
  }
  
  //Get the DevId index and add DevIdTag to packet
  //==============================================
  ...
  for (uint32_t i = 0; i < n->GetNDevices (); ++i) {
  	if (n->GetDevice(i) == this){
		devindex = i;
		//std::cout << "WifiNetDevice Dev Index: " << devindex << std::endl;
	}
  }
  
  //Then append the DevIdTag and append to the packet
  if ((packet->PeekPacketTag(qosTag)) && (packet->PeekPacketTag(frametypeTag))){
	DevIdTag devIdTag;
	devIdTag.SetDevId(devindex);
	packet->AddPacketTag(devIdTag);	
  }
  ...
  //std::cout << "WifiNetDevice PacketID: " << packet->GetUid() << std::endl;		//NK
