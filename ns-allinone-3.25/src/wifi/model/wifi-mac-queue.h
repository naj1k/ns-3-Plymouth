...
public:
  ...
  std::string GetViContents (uint8_t tid, WifiMacHeader::AddressType type, Mac48Address addr);
  std::string ConvertInt(uint64_t packetuid);
  bool RemoveBFrameByPort(uint32_t port, uint32_t seqid);
  bool RemoveAnyBFrame();
  bool RemoveBFrameByAppId(uint32_t AppId);
  
  bool IBoost ();
  bool IBoostByAppId (uint32_t SrcAppId);
  ...
  //Setters and Getters for CheckPacketInfo()
  ...
  //Functions to log events where packets are dropped
  void Log_PacketDrop(Ptr<const Packet> packet, std::string Reason);
